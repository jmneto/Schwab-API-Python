...
# Add the local schwabdev package to the path
...
# print credentials/config to console
...
order = {
    ...
    "price": "10.00",
    "orderLegCollection": [{"instruction": "BUY", "quantity": 1, "instrument": {"symbol": "INTC", "assetType": "EQUITY"}}],
}
...
# print(client.price_history("AAPL", periodType="day", frequencyType="minute", frequency=1,
# startDate=datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=15),
# endDate=datetime.datetime.now(datetime.timezone.utc)).json())
response = client.price_history("SMCI", periodType="day", period=1, frequencyType="minute", frequency=1).json()
print(json.dumps(response, indent=2))
with open("price_history_output.json", "w") as f:
    json.dump(response, f, indent=2)
print(f"\nResponse written to price_history_output.json")
